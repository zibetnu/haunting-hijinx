...
func _ready() -> void:
	...
	var hunter_peer_id: PeerID = hunter.get_node_or_null(^"%PeerID")
	if hunter_peer_id != null:
		hunter_peer_id.id = -1

	var hunter_name_label: Label = get_node_or_null(^"%NameLabel")
	if hunter_name_label != null:
		hunter_name_label.hide()

	var hunter_alert: Alert = hunter.get_node_or_null(^"%Alert")
...
func _apply_camera_limits() -> void:
	var camera: Camera2D = ghost.get_node_or_null(^"%Camera2D")
	if camera == null:
		return
