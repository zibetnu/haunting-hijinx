...
func _process(delta):
	...
	_handle_input(delta)
	if _is_press and _current_duration >= hold_duration != _is_hold:
...
func reorder_child(from: int, to: int) -> Error:
	if _is_hold:
		return ERR_LOCKED

	var child := get_child(from) as Control
	if child == null:
		return FAILED
...
func _on_stop_dragging():
	var error: Error = reorder_child(_focus_child.get_index(), _drop_zone_index)
	assert(error == OK)
	_focus_child = null
	_drop_zone_index = -1
	if _is_smooth_scroll:
